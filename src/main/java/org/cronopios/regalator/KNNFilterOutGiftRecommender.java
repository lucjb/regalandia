...
public class KNNFilterOutGiftRecommender<T> extends
		KernelRegressionBasedGiftRecommender<T> {

	private KNNRetriever<GiftPoint<T>> knnRetriever;

	public KNNFilterOutGiftRecommender(List<T> gifts, final Metric<T> metric) {
		super(gifts, metric);
		this.setKnnRetriever(new MetricBasedFullScanKNNRetriever<GiftPoint<T>>(
				this.getPoints(), new Metric<GiftPoint<T>>() {
	...
	public Set<GiftRecommendation<T>> recommend(
		...
		for (GiftRecommendation<T> giftRecommendation : previousRecommendations) {
			if (giftRecommendation.getUserScore().equals(0d)) {
				List<GiftPoint<T>> retrieve = this.getKnnRetriever().retrieve(
						this.getGiftSpace().get(giftRecommendation.getGift()),
						10);
				GiftPoint<T> giftPoint2 = retrieve.get(9);
				double minDistance = this.getMetric().compute(
						giftPoint2.getGift(), giftRecommendation.getGift());
				for (GiftPoint<T> giftPoint : this.getPoints()) {
					double distance = this.getMetric().compute(
							giftPoint.getGift(), giftRecommendation.getGift());
